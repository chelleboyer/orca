...
### AC1: User Invitation System
- [ ] Facilitators can invite users via email through `/api/v1/projects/{id}/invite` endpoint
- [ ] Invitation includes role assignment: facilitator, contributor, or viewer
- [ ] System generates unique invitation tokens with 7-day expiration
- [ ] Invitation emails contain project details and acceptance link
- [ ] Non-registered users can accept invitation and register simultaneously
- [ ] Existing users can accept invitation to join project

### AC2: Role-Based Permissions
- [ ] **Facilitator Role**: Full access - manage members, edit all content, delete project
- [ ] **Contributor Role**: Edit access - create/edit OOUX artifacts, view all content
- [ ] **Viewer Role**: Read-only access - view all content, export capabilities only
- [ ] Permission validation on all API endpoints using role-based decorators
- [ ] Frontend hides/shows features based on user's role in current project
- [ ] Unauthorized access attempts return 403 Forbidden with clear error message

### AC3: Project Membership Management
- [ ] Project member list displays all users with roles and join dates via `/api/v1/projects/{id}/members`
- [ ] Facilitators can modify user roles through PATCH `/api/v1/projects/{id}/members/{user_id}`
- [ ] Facilitators can remove users from projects (except themselves if only facilitator)
- [ ] Users can leave projects voluntarily via DELETE `/api/v1/projects/{id}/members/me`
- [ ] Last facilitator cannot be removed unless transferring ownership
- [ ] Membership changes trigger email notifications to affected users

### AC4: Invitation Management
- [ ] Pending invitations display in project member list with "Pending" status
- [ ] Facilitators can resend invitations via POST `/api/v1/projects/{id}/invitations/{id}/resend`
- [ ] Facilitators can revoke pending invitations via DELETE `/api/v1/projects/{id}/invitations/{id}`
- [ ] Invitation acceptance creates project membership and marks invitation as used
- [ ] Expired invitations are automatically cleaned up and marked as expired
- [ ] Invitation tracking includes sent date, accepted date, and invited by user

### AC5: Access Control Integration
- [ ] All project-related endpoints validate user membership and role permissions
- [ ] Project context middleware injects current user's role for request processing
- [ ] Database queries automatically filter content based on user access level
- [ ] API responses include user's role and permissions for frontend state management
- [ ] Audit log tracks all permission changes and access attempts
- [ ] Role changes take effect immediately without requiring re-authentication
